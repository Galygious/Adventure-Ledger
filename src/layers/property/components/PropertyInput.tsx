// src/layers/property/components/PropertyInput.tsx
...
export default function PropertyInput({ property, value, error, onChange }: PropertyInputProps) {
  const renderInput = () => {
    switch (property.type) {
      ...
      case 'date':
        return (
          <input
            ...
            min={property.minDate?.toISOString().split('T')[0]}
            max={property.maxDate?.toISOString().split('T')[0]}
            className={`w-full rounded-md shadow-sm ${
              error ? 'border-red-300' : 'border-gray-300'
            } focus:border-indigo-500 focus:ring-indigo-500`}
          />
      ...
      case 'tags':
        const tags = Array.isArray(value) ? value : []; // Ensure tags is an array
        return (
